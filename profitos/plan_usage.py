...
def usage_count(metric, organization_id=None, period=None):
    org = current_org()
    org_id = organization_id or (org["id"] if org else None)
    if not org_id:
        return 0
    ...
    try:
        ...
        row = c.execute(
            "SELECT usage_count FROM plan_usage WHERE organization_id=? AND period=? AND metric=?",
...
def quota_allowed(metric, organization_id=None, plan=None):
    return quota_state(metric, organization_id=organization_id, plan=plan)["allowed"]


def record_usage(metric, organization_id=None, amount=1, period=None):
    # Incrémente uniquement après une opération réussie.
    org = current_org()
    org_id = organization_id or (org["id"] if org else None)
    if not org_id:
    ...
    amount = int(amount)
    if amount <= 0:
        return usage_count(metric, organization_id=org_id, period=period)
    ...
    try:
        ...
        c.execute(
            '''
            INSERT INTO plan_usage(organization_id,period,metric,usage_count,updated_at)
            VALUES(?,?,?,?,?)
            ...
            ''',
            (org_id, period, metric, amount, now()),
        )
        c.commit()
        row = c.execute(
            "SELECT usage_count FROM plan_usage WHERE organization_id=? AND period=? AND metric=?",
