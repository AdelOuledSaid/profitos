...
⚠️ Le mode PostgreSQL n'a pas pu être testé en conditions réelles dans cet
environnement de développement (pas d'accès réseau pour installer psycopg2
ni pour se connecter à un serveur Postgres). La traduction SQL couvre tous
les cas identifiés dans app.py au moment de l'écriture, mais un test complet
sur une vraie instance Postgres est nécessaire avant mise en production
(voir MIGRATION_POSTGRES.md).
"""
import os
import re
import sqlite3
...
def connect_auth(sqlite_path):
    if USE_POSTGRES:
        return PGConnection(DATABASE_URL, schema=None)  # schéma public par défaut
